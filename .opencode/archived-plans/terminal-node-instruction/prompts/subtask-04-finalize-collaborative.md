...
# Subtask 04: Update plan-collaborative/prompts/finalize.md — Terminal Node Constraint

## Objective

Add a constraint to `planning/plan-collaborative/prompts/finalize.md` instructing the planning agent that the **final node of the session plan it generates must NOT include a `next` field**. Executing agents rely on the absence of `next` to know they must call `close_session()`. If a generated plan's last node has `next`, the session cannot close.

## Scope

**File:** `planning/plan-collaborative/prompts/finalize.md`
...
- [ ] Read `planning/plan-collaborative/prompts/finalize.md`
