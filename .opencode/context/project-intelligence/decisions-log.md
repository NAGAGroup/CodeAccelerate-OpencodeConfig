...
## Decision: Adopt OpenAgentsControl as Config Base

**Date**: 2026-03-03
**Status**: Decided

### Context
Previously maintained a fully handrolled OpenCode configuration. This worked but required significant effort to build and maintain agent prompts, workflows, context systems, and plugin integrations from scratch.

### Decision
Switched to [OpenAgentsControl (OAC)](https://github.com/darrenhinde/OpenAgentsControl) as the upstream base. This repo now provides only personal overrides and extensions on top of OAC.

### Rationale
OAC is a mature, community-maintained base with a well-designed multi-agent system, context resolution strategy, and workflow patterns. Handrolling duplicated that work with less quality.

### Alternatives Considered
| Alternative | Pros | Cons | Why Rejected? |
|-------------|------|------|---------------|
| Keep handrolled config | Full control | High maintenance, lower quality | Too much ongoing effort |
| Fork OAC | Control over base | Merge conflicts, maintenance burden | Unnecessary — overrides model is cleaner |

### Impact
- **Positive**: Less maintenance, better upstream agent/workflow behavior, community improvements flow in
- **Negative**: Docs (AGENTS.md etc) are stale and need updating; install model changed to per-project
- **Risk**: Breaking changes in OAC upstream require attention on version bumps (mitigated by `autoupdate: false`)

### Related
- See `living-notes.md` for current mid-transition state
