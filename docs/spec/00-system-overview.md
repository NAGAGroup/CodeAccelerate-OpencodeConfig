...
This system was designed from the ground up to function on small models. The minimum viable target is **Qwen3-8B** (or equivalent capability at that parameter scale). Every design decision in this spec — enforcement sequences, prose-based prompts, explicit tool naming, GrepAI-first search, skill-based methodology teaching, semantic notes for cross-node memory — exists because it measurably improves small model reliability.
...
The semantic notes system is Qdrant running locally with FastEmbed. It exists because the system operates under the memory-is-forbidden principle: agents cannot retain information across conversation turns and each invocation is stateless. Small models hallucinate persistence when context is dropped, so the notes system is not optional memory but the system's only source of persistent state. Planning and execution agents share a single Qdrant collection named after the plan (`{plan_name}`). This shared collection serves as their primary communication channel: planning nodes write findings, user decisions, and architecture rationale as they work; execution nodes read this entire collection at kickoff to reconstruct planning context, then write progress and discoveries as they proceed. Every node must read before it writes, ensuring execution agents never act on assumptions about planning discoveries. The collection is a shared ledger, not a cache, preventing the common failure mode where execution agents proceed without understanding what planning discovered. This design also enables recovery from context truncation: if a conversation is compressed or truncated, the next node recovers state from the collection rather than fragile conversation memory.

### How Planning and Execution Share the Collection
During planning, when HeadWrench calls `choose_plan_name`, the plugin creates a Qdrant collection with that name. As the planning DAG runs, each node stores its findings to the collection before advancing. The dag-designer writes the execution DAG to disk, but it does not write semantic notes — those are written by planning nodes as they work. When execution begins, the execution-kickoff node reads the entire collection to reconstruct the planning context. Each execution node then writes its own notes, building a chronological record of the work. This shared collection means that planning and execution are not sequential handoffs; they are continuous phases of a single session. The notes system prevents the common failure mode where execution agents proceed without understanding what planning discovered. It also enables the framework to recover from context truncation: if a conversation is compressed or truncated, the next node can recover state from the collection rather than from fragile conversation memory.

### The Semantic Notes System (Qdrant)

The semantic notes system is a local Qdrant instance powered by FastEmbed that provides persistent, vector-indexed storage for the CodeAccelerate framework. It exists because the system operates under the memory-is-forbidden principle: agents cannot retain information across conversation turns and each invocation is stateless. Small models hallucinate persistence when context is dropped, so the notes system is not optional memory but the system's only source of persistent state. Without it, agents would repeatedly rediscover what was already learned, making multi-step workflows inefficient and error-prone.

The system is used by both planning and execution agents through a single shared Qdrant collection named after the plan (`{plan_name}`). This collection serves as their primary communication channel: planning nodes write findings, user decisions, and architecture rationale as they work; execution nodes read this entire collection at kickoff to reconstruct planning context, then write progress and discoveries as they proceed. Every node must read before it writes, ensuring execution agents never act on assumptions about planning discoveries. The collection is a shared ledger, not a cache, preventing the common failure mode where execution agents proceed without understanding what planning discovered. This design also enables recovery from context truncation: if a conversation is compressed or truncated, the next node can recover state from the collection rather than from fragile conversation memory.
