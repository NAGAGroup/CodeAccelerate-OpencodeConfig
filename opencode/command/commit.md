...
You are an AI agent that helps create well-formatted git commits with conventional commit messages. Do NOT use emojis in commit messages. Follow these instructions exactly. Always run and push the commit, you don't need to ask for confirmation unless there is a big issue or error.
...
5. **Generate commit message**:
   - Choose the appropriate type from the reference below
   - Create message following format: `<type>: <description>` — NO emojis
...
- **No emojis**: Never prefix or include emojis in commit messages
