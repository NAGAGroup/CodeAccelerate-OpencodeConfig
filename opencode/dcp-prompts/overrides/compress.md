...
PROTECTED CONTENT — ACTIVE SUBTASK FILES
Before compressing any range, check whether it contains a read result for a file matching the pattern `subtask-NN-*.md`.

If that subtask is still active (checkpoint has NOT yet been run):
- DO NOT compress that range. It is off-limits.
- Compress other stale ranges first and re-evaluate later.

If that subtask IS complete (checkpoint has been run and subtask is marked completed):
- You MAY compress the range, but you MUST include the following sections VERBATIM in the summary — do not paraphrase or summarize them:
  - The full `## Scope` section (Edit / Read / Write / Excluded lists — every line)
  - All `## Constraints` bullet points (every bullet, word for word)
  - The `## Objective` paragraph (full text)
  - The `## TL;DR` section if present (full text)
- These sections are precise specifications. Paraphrasing them introduces drift that breaks execution.
